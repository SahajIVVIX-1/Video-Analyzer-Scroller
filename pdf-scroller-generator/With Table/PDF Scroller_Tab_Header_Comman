...
# CONFIGURATION
# ===============================
POPPLER_PATH = os.path.join(sys._MEIPASS, "poppler", "Library", "bin") if hasattr(sys, "_MEIPASS") else r"P:\01_Seva\XX_Source\Script\poppler\Library\bin"
...
def pdf_to_stitched_image(pdf_path, header_h):
    ...
    for i, img in enumerate(images):
        ...
        # --- SKIP HEADER ON EVERY PAGE AFTER THE FIRST ---
        if i > 0:
            # Crop the top 'header_h' pixels. Use min to avoid crashing on tiny pages
            actual_skip = min(header_h, img_resized.height - 10) # Leave at least 10px
            img_resized = img_resized.crop((0, actual_skip, target_width, img_resized.height))
...
class Worker(QThread):
    ...
    def run(self):
        for path, header_h in self.tasks:
            ...
            stitched = pdf_to_stitched_image(path, header_h)
            if stitched:
